/**
 * Detail component that displays movie or actor information based on URL parameters.
 * 
 * @component
 * @returns {React.ReactElement} A detailed view of either a movie with its actors or a specific actor's biography.
 * 
 * @description
 * This component handles two main views:
 * 1. Movie Detail View: Shows movie poster, summary, and a grid of actors with links to their individual profiles.
 * 2. Actor Detail View: Shows actor image and biography when accessed through a movie's actor list.
 * 
 * Uses React Router's useParams to extract `idPeli` (movie ID) and `idInterprete` (actor index) from the URL.
 * Falls back to error messages if the requested movie or actor is not found.
 * 
 * @throws Renders error message if movie with given `idPeli` is not found.
 * @throws Renders error message if actor with given `idInterprete` index is not found.
 * 
 * @example
 * // Movie detail view
 * <Detail /> // when URL is /detalle/pelicula/1
 * 
 * @example
 * // Actor detail view
 * <Detail /> // when URL is /detalle/pelicula/1/interprete/0
 * 
 * @see {@link List} - Used to display individual actor cards
 */
